...
- **ZotUI environments now have a standard runtime when `image` is omitted.** Local Docker and the repository Dev Container use the same pinned Microsoft Dev Containers Bookworm base. On Vercel, ZotUI omits both `image` and the legacy `runtime` property so the `/v3/sandboxes` endpoint selects Vercel's managed default image. All provide a shell, Git, and curl, and ZotUI deploys Zot separately. `image` remains an explicit compute-specific toolchain override; Vercel overrides must be a Vercel-managed image or reside in the configured project's VCR.
