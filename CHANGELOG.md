...
- **ZotUI environments now have a standard runtime when `image` is omitted.** Local Docker and the repository Dev Container use the same pinned Microsoft Dev Containers Bookworm base, while Vercel requests its built-in `node24` runtime because it cannot pull external registry images. All provide a shell, Git, and curl, and ZotUI deploys Zot separately. `image` remains an explicit compute-specific toolchain override; Vercel overrides must reside in the configured project's VCR.
