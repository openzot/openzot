...
// for the zot binary. When built via `go build`/`go run` without setting the
// Version variable, it defaults to "dev" and all update checks are skipped.
package version
...
// constant purely so a test can point the check at a local server; nothing at
// runtime changes it.
var APIBase = defaultAPIBase

// Version is the semver tag (e.g. "v0.1.0"), set at build time via -ldflags.
// Defaults to "dev" when built without them (i.e. via `go run`).
var Version = "dev"
...
// LatestRelease queries the GitHub API for the latest published release of the
// zot repository. Returns the tag name, the release URL, and any error.
...
// parseSemver splits a "major.minor.patch" string into three integers. Returns
// nil if the string is not a valid semver triple.
...
// current Version. If Version is "dev" (i.e. not a release binary), the check
// is skipped and a nil result is returned.
